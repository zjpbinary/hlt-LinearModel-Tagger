            ...
            for i in range(len(self.sentset)):
                for p in range(len(self.sentset[i])):
                    ...
                    if maxtag != self.tagset[i][p]:
                        ...
                        fcorrect = self.mergefeature(f, self.tagset[i][p])
                        ferror = self.mergefeature(f, maxtag)
                        for k in ferror:
                            self.weight[k] -= iterator
                        for k in fcorrect:
                            self.weight[k] += iterator
            precision = self.evaluate()
            print('第%d次迭代'%iterator, '精度为%f'%precision)
    def evaluate(self):
        count = 0
        right = 0
        for i in range(len(self.sentset)):
